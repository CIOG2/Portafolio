...
const Template: FC<Props> = ({ title, color, tag, permanent, content, id}) => {  
  const articleContainer = useRef<HTMLElement>(null); 
  

  const deleteTemplate = () => {
    if(articleContainer.current !== null)
      articleContainer.current.remove();
  }
  ...
  const contentValue = (value: string) => {
    if (value !== "") {
      ...
      return (typeof(arrayValue) === 'object') 
        ? arrayValue.map((e: string) => `${e}\n`).join('')
        //En este caso seria un String
        : arrayValue;      
    }
  ...
  return (
    <article 
      className = {`${Styles['article__container']} ${'articles-container'}`} 
      ref = {articleContainer}
      style ={{backgroundImage: color}}
    >
      <header className = {Styles['article__container--header']}>
        ...
        <div className = {Styles['container--header-settings']}>
      ...
      <main className = {Styles['article__container--main']}>
        {('Imagen Principal' === title || 'Imagen' === title )
          //Input file
          ?(
            <LoadImage
              title = {title}
          ...
          :(
            <textarea 
              className = {Styles['container--main-textarea']}
              id = {title.split(" ").join("-")}
